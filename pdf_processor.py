...
def extract_page_clean(page) -> str:
    ...
    4. Standalone page numbers remove (1, 2 ... 156)
    """
...
def extract_pdf_pages(pdf_path: str):
    """
    Saari pages extract karo.
    Returns: (pages_list, total_pages)
    pages_list = [{"page_number": N, "content": "..."}]
    """
    try:
...
# ==========================================
# GET PAGE RANGE — Summary ke liye
# ==========================================
...
# ==========================================
# BM25 SEARCH — Vectorless RAG
# ==========================================
def bm25_search(book_id: int, query: str, top_k: int = 4):
    """
    BM25 se query ke liye relevant pages dhundo.
    Returns top-k pages ka combined context string.
    """
    try:
        rows = get_all_pages_from_db(book_id)
        if not rows:
            logging.warning(f"⚠️ No pages in DB for book {book_id}")
            return None
        ...
        # Fallback: score 0 pe bhi top 2 lo
        if not relevant:
        ...
        logging.info(
            f"✅ BM25 book={book_id} query='{query[:40]}' "
            f"pages={[r['page_number'] for r in relevant]}"
        )
        return context

    except Exception as e:
        logging.error(f"❌ BM25 error: {e}")
        return None
...
if __name__ == "__main__":
    ...
    print("   Fix 4: Standalone page number removal")
